...
public class SpruceDryingRackRenderer implements BlockEntityRenderer<SpruceDryingRackEntity> {

    public SpruceDryingRackRenderer(BlockEntityRendererFactory.Context context) {
    }

    @Override
    public void render(SpruceDryingRackEntity entity, float tickDelta, MatrixStack matrixStack, VertexConsumerProvider vertexConsumers, int light, int overlay) {
        
        ItemRenderer itemRenderer = MinecraftClient.getInstance().getItemRenderer();
        ItemStack itemStack = entity.getStack(0);
        Direction dir = (Direction)entity.getCachedState().get(SpruceDryingRack.FACING);
